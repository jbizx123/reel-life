...
export default function HomeScreen() {
  ...
  // Handle payment-success deep link: reellife://payment-success?project_id=...&tier=...
  useEffect(() => {
    const handleUrl = (event: { url: string }) => {
      console.log('[Home] Deep link received', event.url);
      try {
        ...
        if (host === 'payment-success') {
          ...
          console.log('[Home] Payment success deep link', { projectId, tier });
          if (projectId) {
            const renderType = tier === 'poster' ? 'poster' : 'full_movie';
            router.push({
              pathname: '/render-progress',
              params: { project_id: projectId, type: renderType },
            });
          }
        }
      } catch (e) {
        console.log('[Home] Failed to parse deep link URL', e);
      }
    };

    const subscription = Linking.addEventListener('url', handleUrl);

    // Also check if the app was opened from a cold start via the deep link
    Linking.getInitialURL().then(url => {
      if (url) {
        console.log('[Home] Initial URL on mount', url);
        handleUrl({ url });
      }
    });
